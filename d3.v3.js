(function() {
		...
		d3.xhr = function(url, mimeType, callback) {
			var xhr = {}, dispatch = d3.dispatch("progress", "load", "error"),
				headers = {}, response = d3_identity,
  ...
  function d3_ease_bounce(t) {
    return t < 1 /
						2.75 ? 7.5625 * t * t : t < 2 / 2.75 ? 7.5625 * (t -= 1.5 / 2.75) * t + .75 : t < 2.5 / 2.75 ? 7.5625 * (t -= 2.25 / 2.75) * t + .9375 : 7.5625 * (t -= 2.625 / 2.75) * t + .984375;
					}
					d3.event = null;
				...
				function squarify(node) {
					...
					if (children && children.length) {
						var rect = pad(node),
							...
							child, best = Infinity,
							score, u = mode === "slice" ? rect.dx : mode === "dice" || mode === "slice-dice" && node.depth & 1 ? rect.dy : Math.min(rect.dx, rect.dy),
							n;
  ...
  var d3_geo_azimuthalEquidistant = d3_geo_azimuthal(function(cosλcosφ) {
    ...
    return c && c /
				...
				}).raw = d3_geo_azimuthalEquidistant; d3.geo.bounds = d3_geo_bounds();
				...
				function d3_geo_clipSort(a, b) {
					return ((a = a.point)[0] < 0 ? a[1] - π/ 2 - ε : π /
						2 - a[1]) - ((b = b.point)[0] < 0 ? b[1] - π/ 2 - ε : π /
						2 - b[1]);
				}
					...
					return {
						...
						point: function(λ1, φ1) {
							var sλ1 = λ1 > 0 ? π : -π,
								dλ = Math.abs(λ1 - λ0);
							if (Math.abs(dλ - π) < ε) {
								listener.point(λ0, φ0 = (φ0 + φ1) / 2 > 0 ? π/ 2 : -π /
									2);
      ...
      φ = direction * s /
				...
				}).raw = d3_geo_gnomonic; d3.geo.graticule = function() {
				...
				function d3_geo_mercator(λ, φ) {
					return [λ/ (2 * π), Math.max(-.5, Math.min(+.5, Math.log(Math.tan(π /
						4 + φ/ 2)) / (2 * π)))];
		}
  ...
  var d3_geo_pathAreaSum, d3_geo_pathAreaPolygon, d3_geo_pathArea = {
    ...
    polygonEnd: function() {
      ...
      d3_geo_pathAreaSum += Math.abs(d3_geo_pathAreaPolygon /
				2);
		}
	};
	...
	function d3_geo_areaRingStart() {
		varλ00, φ00, λ1, λ0, φ0, cosφ0, sinφ0;
		...
		function nextPoint(λ, φ) {λ *= d3_radians, φ *= d3_radians;
			if (Math.abs(Math.abs(φ0) - π/ 2) < ε && Math.abs(Math.abs(φ) - π /
				2) < ε) return;
			var cosφ = Math.cos(φ),
				sinφ = Math.sin(φ);
			if (Math.abs(φ0 - π/ 2) < ε) d3_geo_areaRing += (λ - λ1) * 2; else {
        var dλ = λ - λ0, cosdλ = Math.cos(dλ), d = Math.atan2(Math.sqrt((d = cosφ * Math.sin(dλ)) * d + (d = cosφ0 * sinφ - sinφ0 * cosφ * cosdλ) * d), sinφ0 * sinφ + cosφ0 * cosφ * cosdλ), s = (d + π + φ0 + φ) /
					4; d3_geo_areaRing += (dλ < 0 && dλ > -π || dλ > π ? -4 : 4) * Math.atan(Math.sqrt(Math.abs(Math.tan(s) * Math.tan(s - d / 2) * Math.tan(s - π/ 4 - φ0 /
						2) * Math.tan(s - π/ 4 - φ /
						2))));
				}
		...
		function d3_geo_forwardRotationλ(δλ) {
			return function(λ, φ) {
				returnλ +=δλ, [λ > π ? λ-2 * π : λ < -π ? λ+2 * π : λ, φ];
			};
		}
